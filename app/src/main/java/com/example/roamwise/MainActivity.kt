...
import com.example.roamwise.stats.getTotalMobileBytesForSubscription
...
class MainActivity : AppCompatActivity() {
    ...
    private var debugForceRoaming = false
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        binding.title.setOnLongClickListener {
            debugForceRoaming = !debugForceRoaming
            val msg = if (debugForceRoaming) {
                "Debug: Force Roaming = ON"
            } else {
                "Debug: Force Roaming = OFF"
            }
    ...
    private fun refreshUi() {
        ...
        lifecycleScope.launch {
            ...
            if (Build.VERSION.SDK_INT >= 28 && hasUsage) {
                ...
                if (s1 != null) {
                    val used1 = if (debugForceRoaming) {
                        getTotalMobileBytesForSubscription(this@MainActivity, s1.subscriptionId, start)
                    } else {
                        getRoamingBytesForSubscription(this@MainActivity, s1.subscriptionId, start)
                    }
                    binding.sim1Panel.visibility = android.view.View.VISIBLE
                    binding.sim1Title.text = "SIM 1 • ${s1.displayName ?: s1.carrierName ?: "Unknown"}" +
                            if (debugForceRoaming) "  [DEBUG]" else ""
                ...
                if (s2 != null) {
                    val used2 = if (debugForceRoaming) {
                        getTotalMobileBytesForSubscription(this@MainActivity, s2.subscriptionId, start)
                    } else {
                        getRoamingBytesForSubscription(this@MainActivity, s2.subscriptionId, start)
                    }
